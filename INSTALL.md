...
1. First install *Simple SMTP*: `apt install ssmpt`
...
To test that it works, create and execute a script callled `test-ssmtp.sh` with a content like this:
```
#!/bin/bash -x
...
Note: Replace `user@example.org` with your email address or call the
script with your email address as an argument.
...
addresses. If a spammer tries to send a mail as it is coming from your
domain, the SMTP server that is getting this email will consult this
DNS record, will figure out that the server of the spammer is not
...
spams and faked emails. If a mail server signs its messages it is less
likely that they will classified as spam.
...
- On the DNS configuration of the domain add a TXT record like this:
  ...
  ```
  The value of the txt record is the one generated by the website
  above.

- You can check that it has been activated, try the command:
  ```
...
- Try to send a test email from the host (outside the container):
  ...
  trusted hosts (that are allowed to send email for the domain
...
- Send an email to `check-auth@verifier.port25.com`:
  ...
  status and helth of your email server (for example whether the mails
...
The same server can support more than one mail domains. If we want to
add another mail domain, for example `example.com`, we have to do
these:

- Edit `config/virtual_alias_domains` and add it on a new line.

- Edit `config/virtual_alias_maps.cf` and new email aliases (for
...
- Update the DNS configuration with records like these:
  ...
  + The value of the DMARC record is the one obtained on the previous
    step.
  You can check these DNS configurations with commands like these:
  ```
...
be on the list `config/trusted_hosts` on the mail server, otherwise it
will refuse to send emails. After adding it on this list, run `ds
inject update.sh` to update the configuration of the mail server.
...
But we can also configure it from command line, like this:
```
